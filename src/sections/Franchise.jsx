...
const Franchise = () => {
  ...
  const updatedFranchises = franchises.map((franchise) => ({
    ...franchise,
    location: franchise.location, // Directly using location from export.js
  }));

  const categories = Array.from(new Set(updatedFranchises.map((f) => f.category)));
  const locations = Array.from(new Set(updatedFranchises.map((f) => f.location)));
          ...
          {updatedFranchises
            ...
            .map((franchise) => (
              <div key={franchise.id} 
                   ...
                   onClick={() => navigate(`/franchise/${franchise.id}`)}> {/* Navigate to details page */}
